...
//  - 영양(단백질·칼로리·보충제)은 다루지 않는다(2026-09-02 결정) — 코치 원칙 5번이 훈련 외 주제로 넘긴다.
//  - 2026-09-02 재검증: docs/research/v2-*.md · 선별안 docs/research/v2-selection-plan.md
//  - 사용자 개인 데이터(무게/볼륨 등)는 여기 넣지 말 것 — 그건 가변 컨텍스트에서 주입됨.
// ═══════════════════════════════════════════════
var COACH_KNOWLEDGE =
  '### 1. 트레이닝 핵심 원리\n' +
  '- 볼륨: 부위당 주 10~20세트(환산 기준)가 핵심 구간. 세트 1개당 약 +0.24% 근비대, 수확 체감은 있지만 금지선이 아니다 — 주 31세트까지도 검출 가능한 추가 이득이 있었다 (Pelland 2026, Sports Med, 67연구·2,058명). 4세트 미만=부족(검출 가능한 최소 용량), 5~10세트 구간이 세트당 효율이 가장 좋다. 간접(보조근) 세트는 0.5로 환산해 센다 — 세 방식(0/0.5/1.0) 중 통계적으로 예측력이 가장 높다(Pelland 2026). 근비대는 환산 세트로, 근력은 직접 세트로 센다. 반대 근거도 있다: 훈련자 12주 RCT에서 주 9 vs 36 환산 세트가 동등(Steele 2026) — 목표 미달을 실패로 말하지 않는다.\n' +
  '- 작은 근육(측면·후면 어깨, 이두, 삼두, 종아리)의 목표를 낮추지 않는다. 간접 세트는 이미 0.5로 더해져 있으므로 "복합으로 충분"이라는 결론을 내리지 말 것. 이 부위들은 직접 세트(고립)가 주 4세트 이상 필요하다 (컬 > 로우 약 2배: Mannarino 2021 / 종아리 12 > 6세트: Kassiano 2024).\n' +
  '- 빈도: 주간 총 세트가 같으면 근비대는 주 1회든 2회든 차이가 없다 (Schoenfeld·Grgic·Krieger 2019, 25개 연구 — 2016년 메타의 같은 저자들이 뒤집음; Pelland 2026도 동일). 근력에는 빈도가 유의하다(Pelland 2026). 그럼에도 주 2회를 권하는 이유는 세트를 나눠 담는 그릇이 필요해서다 — 세션당 약 11 환산 세트부터 추가 이득이 관측되지 않는다(Remmert 2025). 사용자가 주 1회 분할을 선호하면 막지 말고 세션당 부위 세트 수만 점검한다.\n' +
  '- 강도(노력): 본세트는 실패 0~3회 전(RIR 0~3, 기본 1~3)에서 멈춘다. 실패에 가까울수록 근비대는 소폭 늘고 근력은 무관하다(Robinson 2024 메타회귀). 부하가 가벼울수록 실패에 더 가까이 간다 — 고립·머신 RIR 0~2, 복합 RIR 2~3. 무거운 바벨 복합의 완전 실패는 피로만 누적하고 근력 향상까지 해칠 수 있다(Refalo 2023). RIR은 자가 보고라 부정확하므로 통과/탈락 게이트가 아니라 소프트 기준으로 쓴다. ACSM 2026 포지션 스탠드(Currier 외, 137개 체계적 고찰)도 %1RM 대신 RIR을 1차 강도 지표로 채택했다.\n' +
  '- 반복 범위: 약 5~30회 모두 실패에 근접하면 근비대 효과는 동등 (Schoenfeld·Grgic 2021; ACSM 2026은 1RM의 30~100%). 앱 기본은 메인 복합 6~10 · 보조 복합 8~12 · 고립 10~15 · 소근육 고립 12~25. 저반복=근력에 유리, 고반복=관절 부담↓.\n' +
  '- 신장 강조(스트레치): 근육이 늘어난 위치에서 외부 저항이 실제로 걸리는 종목·가동범위가 근비대에 유리 (2026 메타 8편 ES 0.28; Pedrosa·Kassiano 2026 리뷰). 늘어난 위치에 저항이 없으면(예: 저항이 위쪽에 몰린 컬) 길이만 늘려도 이득이 없다. 신장 부분반복이 전가동보다 낫다는 근거는 2025년 이후 "동등"으로 수렴 — 전가동으로 신장 구간까지 쓰면 충분하다. 효과 크기가 작으니 단정하지 말고 "저비용 헤지"로 말한다.\n' +
  ...
  '- 주기화: 앱은 5주 사이클(빌드 4주 + 디로드 1주)을 리듬으로 쓴다. 주기화 유형(선형·파동)은 근비대에 차이를 만들지 않는다(Grgic 2017 메타 SMD −0.02) — 사이클은 성장 지렛대가 아니라 피로·진행 관리 리듬이다. 주차는 날짜가 아니라 그 주 목표 운동 완료로 넘어감.\n\n' +

  '### 2. 종목 자세(폼) 핵심 큐 + 흔한 실수\n' +
  '- 공통: 본세트 전 점진적 워밍업, 통제된 신장(eccentric) 1~3초, 가동범위는 신장 위치까지 충분히. 반동/치팅으로 무게를 올리는 것은 자극을 분산시킴. 단 통제된 신장은 근비대 처방이 아니라 반동을 없애는 폼 큐다 — 템포·TUT는 근비대에 일관된 영향이 없고 8초 초과만 열등하다(2025 템포 메타, ACSM 2026).\n' +
  ...
  '- 측면/후면 레터럴 레이즈: 가벼운 무게로 통제, 승모근으로 으쓱 들지 말 것. 소근육은 다회수가 적합. 후면 삼각근은 프레스·로우로 대체되지 않으므로 직접 고립(리버스 펙덱·리어 레터럴)이 필요하다(EMG·합의 근거). 측면 삼각근은 덤벨과 케이블이 동등하다(Larsen 2025).\n' +
  '- 이두: 인클라인 컬은 근위(어깨 쪽), 프리처 컬은 원위(팔꿈치 쪽)가 더 자란다(Kassiano 2025) — 경쟁이 아니라 분업. 팔이 정체하면 둘 다 넣는다.\n\n' +
  ...
  '- 허리: 굴곡/회전 부하에서 통증 흔함 → 힙힌지 패턴 점검, 복압·중립 유지, 일시적으로 무게↓. 앱이 데드리프트·바벨 스쿼트류를 기본 차단하는 것은 "위험이 입증돼서"가 아니라 앱이 폼을 볼 수 없어 보수적으로 막는 안전 마진이다 — 부하 상태 반복 굴곡의 손상 기전은 시체·동물 모델 근거이고, 생체 코호트 메타(Saraceni 2020)는 굴곡–요통 연관을 못 찾았으며, 데드리프트는 요통 재활 RCT(Berglund 2015)에도 쓰였다. "허리에 위험해서 금지"라고 단정하지 않는다.\n' +
  '- 무릎: 슬개건 통증 흔함 → 가동범위·템포 조정, 통증 없는 범위에서 볼륨 유지.\n' +
  '- 손목/팔꿈치: 그립·각도(EZ바/뉴트럴) 변경으로 완화되는 경우 많음.\n' +
  '- 통증 후 진행: 앱은 통증 기록 후 14일간 증량을 막는다. 사용자에게는 "2주 금지"가 아니라 "통증이 가라앉고 같은 무게가 아프지 않으면 그때 올려요"로 말한다. 현대 통증-부하 관리는 기간이 아니라 증상 강도와 다음 날 아침 회복 여부로 진행을 정한다(통증 감시 모델 Silbernagel 2007; Smith 2017 메타).\n' +
  '- 근육통과 초기 손상은 성장 신호가 아니다(Damas 2016: 초기 단백질 합성 급등은 손상 복구이고, 근비대와의 상관은 3주 후부터). "안 아프면 헛운동"이 아니라고 답한다.\n' +
  '- 2주 이상 지속·악화되거나 일상에 지장을 주면 의료/물리치료 전문가 상담 권유. 코치는 진단을 대신하지 않음.\n\n' +

  '### 4. 회복 — 수면·디로드·부위 회복\n' +
  '- 수면 7~9시간. 4~6시간으로 줄면 MPS·회복·수행 저하. 단 못 잔 날에도 훈련은 하는 편이 낫다(볼륨만 줄인다) — 운동이 수면 부족의 단백질 합성 저하를 상쇄한다(Saner 2020).\n' +
  '- 부위 회복은 시계가 아니라 다음 세션의 실제 수행으로 판단한다. "소근육 24~36시간 / 대근육 48~72시간"은 참고치일 뿐 근거가 약하다 — 하체 주 2회를 "48시간이 안 지났다"고 막지 않는다.\n' +
  '- 디로드: 종목·빈도·무게는 유지하고 세트 수만 −40~50%, RIR 3~4, 5~7일. 완전 휴식이 아니다. 디로드는 근성장을 늘리지도 깎지도 않는다(Coleman 2024 훈련자 RCT; Pancar 2026 초보 RCT) — 누적 피로·관절·심리 관리용이지 성장 부스터가 아니다. 주기는 4~8주가 권장 범위이고 너무 잦으면 이득을 깎는다(Bell 2025). 디로드 다음 주는 디로드 직전 무게에서 세트를 원래대로 되돌린다.\n' +
  '- 정체 판정: 종목당 4세션·28일 이상 표본이 있을 때만 정체라고 말한다(앱 엔진 기준; 2~3세션 무게 동일은 측정 잡음 범위). 컨텍스트에 "정체"로 표시된 종목만 정체다.\n' +
  '- 컨디션/RPE: RIR 1~3으로 훈련하면 세션 RPE 8~9는 정상값이다. 저항운동에서 비기능적 과훈련 위험은 낮다(Bell 2025). 판단은 절대값이 아니라 개인 평소 대비 급격한 상승(+1.5 이상)과 컨디션 저하가 겹칠 때 한다 — 그때 강도·볼륨을 일시 낮추는 게 장기적으로 이득.\n\n' +
  ...
  '- 본세트 전 정적 스트레칭은 한 근육당 60초 미만이면 넣어도 된다 — 배제할 근거가 없다(Warneke 2024, 83편: <60초 ES −0.18, 뒤에 동적 활동이 이어지면 손실은 무시할 만함). 60초 이상은 피한다(최대근력 −5.4%: Simic 2013, 104편; ≥60초 ES −0.84).\n' +
  '- 운동 후 스트레칭이 근거로 주는 건 **유연성(가동범위) 유지** 하나뿐이다(2025 국제 델파이 합의문, 연구자 20명 전 항목 80%+ 합의). 부위당 30초씩 3~4동작 = ACSM "유지" 기준(근육당 총 60초)은 맞지만, "유연성 향상"(3세트×120초) 기준에는 한참 못 미친다 — 이 차이를 정직하게 말한다. 유연성 목적이라도 세션 4분·주 10분을 넘기면 추가 이득이 없다. 세트 간 스트레칭은 근거가 약하고 하퇴에 국한돼 권하지 않는다.\n' +
  ...
  '- 정체 시 개입 순서는 "검증된 순서"가 아니라 위험·비용 순이다: ① 회복 점검(수면·완수율) → ② 디로드 1주 → ③ 해당 부위 주간 세트 수 조정 → ④ 종목 변경. 볼륨 격차가 명백하면 ③을 ②보다 먼저 한다. 무게 +한 칸 도전(덤벨 2kg·그 외 5kg / 어시스트 종목은 보조 −한 칸)은 ③의 한 형태다.\n' +
  '- 보조 기법(고립·마무리 한정): 드롭셋(실패 후 무게 내려 이어가기 — 세션당 1종목), 슈퍼셋(시간 절약 — 길항쌍만), 레스트포즈(짧은 휴식 후 추가 반복). 메인 복합운동보다는 고립/마무리 세트에 적합하며 회복 비용을 고려해 과용 금지.\n\n' +

  '### 7. 유산소 병행\n' +
  '- 유산소는 근비대를 깎지 않는다 (Schumann 2022 메타 43편 근비대 SMD −0.01; Held 2026 엄브렐라 리뷰 17개 메타·144연구). 남는 손해는 폭발적 근력뿐이고, 회복·작업 능력은 오히려 오른다. "유산소 하면 근손실"이라는 불안에는 사실로 답한다.\n' +
  '- 배치: 다른 날 / 같은 날 / 같은 세션 사이에 근비대 차이가 확인되지 않았다. 지킬 규칙은 둘뿐 — ① 같은 세션이면 웨이트 먼저 → 걷기 나중(하체 수행 볼륨 보호; 근력 SMD 1.69) ② 하체 세션 직전에 걷기를 두지 않고, 하체 다음 날 걷기는 경사 −2% 또는 −5분으로 한 단계 낮춘다(급성 피로: Doma 2019). 상체 날(푸시·풀·상체)엔 같은 세션에 붙여도 무해하다(Sabag 2018).\n' +
  '- 경사 걷기가 웨이트와 궁합이 좋은 이유는 "달리기가 아니라서"가 아니라 비행 구간이 없어 신장성 손상이 작기 때문이다. "달리기가 자전거보다 간섭이 크다"는 지금 논쟁 중이라 단정하지 않는다.\n' +
  '- 걷기 주 4회·주간 +10% 상한은 근비대 근거가 아니라 회복·지속률·안전을 위한 상한이다. 주 250~300분은 체중 감소 기준(ACSM)이고, 근비대 목적의 유산소 상한은 근거가 없다.\n' +
  '- 걷기 강도 RPE는 0~10 척도로 4~6이 정상. 대화 테스트("문장은 되는데 노래는 안 되는" 정도)가 1순위 지표.\n';
