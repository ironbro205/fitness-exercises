...
async function modifyRoutineWithAI(currentRoutine, userRequest, chatHistory) {
  ...
  var systemPrompt = '당신은 과학적 근거 기반의 피트니스 코치다. ' +
    ...
    '6. **무게 — 점진적 과부하 우선**\n' +
    '   - "최근 종목별 실제 수행" 표에 데이터 있으면 그 무게 기준 더블 프로그레션 (상단 횟수 2세션 연속 달성 시 +한 칸[덤벨 2kg·그 외 5kg], 미달 시 동일. 같은 무게로 2세션 연속 하단 미달이면 앱이 한 칸 내린다 — 앱 계산, 그 무게를 그대로 쓴다)\n' +
...
- 참고로 앱의 기본 배정은 이렇다(설명용 — JSON에 넣지 말 것): 고중량 복합 = 탑세트(가장 무거운 1세트 + 백오프 2세트, 탑의 90%(덤벨·한쪽씩·케이블·고립은 85%)), 나머지 = 스트레이트 3세트.
...
- 목표 미달이면 같은 무게 유지. 같은 무게로 2세션 연속 목표 하단 미달이면 앱이 장비 단위 한 칸 내린다(앱 계산 — 너는 그 무게를 그대로 쓴다).
- 한쪽씩 하는 복합 종목(런지·불가리안 스플릿 스쿼트·싱글 레그 데드리프트·원 암 랫 풀 다운)은 반복 범위 8-12.
