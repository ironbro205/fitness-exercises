...
function buildUserContext(options) {
  ...
  if (volumeKeys.length > 0) {
    ...
    ctx += '※ 큰 근육(가슴·등·대퇴사두·햄스트링·둔근) 주 10~20세트 / 작은 근육(어깨·이두·삼두·복근) 주 8~16세트가 적정(작은 근육은 복합운동 간접자극을 받아 목표가 낮다). 큰 근육 20+·작은 근육 16+는 수확 체감. 간접(보조근) 세트는 0.5로 합산\n';
...
- 각 항목 옆의 "목표 N세트까지 N세트 더"를 읽고, 이번 세션의 그 부위 세트 수를 그 격차에 맞춰 정한다(많이 부족할수록 세트를 더, 거의 찼으면 적게). 주간 목표는 부위 크기별로 다르다 — 큰 근육(가슴·등·대퇴사두·햄스트링·둔근) 약 12세트, 작은 근육(어깨·이두·삼두·복근) 약 8세트(복합운동 간접자극을 받아 목표가 낮다).
...
- 그 세션에서 가장 무거운 메인 복합 종목(탑세트 대상 — 핵 스쿼트, 체스트 프레스류 등)을 반드시 1~2번째에 배치한다. 신선할 때 해야 무게·반복 수행이 최대가 되고 폼이 지켜지며(특히 허리 보호), 앞에 한 종목이 근력 이득을 더 가져간다(우선순위 원칙 — Nunes 2021).
...
## 🧬 과학 근거 (요지)
- 주간 볼륨이 근비대 1순위 동력, 부위당 주 10~20세트가 실용 최적(Pelland 2024; Baz-Valle 2022) — 20+는 수확 체감이지 금지선이 아니다. 작은 근육(어깨·이두·삼두)은 복합운동 간접자극을 받아 목표가 약 절반(주 8~16세트)으로 낮다. 종아리는 간접자극이 거의 없어 직접 볼륨이 필요하다(목표를 낮추지 않는다).
