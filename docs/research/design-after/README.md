...
> **되돌린 것 하나 (v54 · PR #62)** — 2단계 처방 표에서 "전 종목이 같은 값이면 세트·RIR 열을
> 접어 머리글 옆에 한 번만"은 종목별 세트 수를 못 읽게 만들어 되돌렸습니다. 값은 항상 종목
> 줄마다 적습니다. 되돌리면서 표가 AI가 준 원본 숫자를 적고 있던 것도 고쳐, 이제 다섯 칸
> 모두 `[시작]`이 실제로 만들 세트에서 나옵니다(무게는 기록 기반 추천, RIR·휴식은 세트 역할과
> 종목 클래스가 정한 값). 고중량 복합에는 `탑세트 1 + 백오프 2 (90% · RIR 2-3)` 구성 줄을
> 더했습니다.
> 후 캡처: [03b-workout-step2-restored.png](03b-workout-step2-restored.png) (PUSH — 03과 같은 화면),
> [03c-workout-step2-topset.png](03c-workout-step2-topset.png) (LEGS — 탑세트+백오프 줄이 보이는 화면).
