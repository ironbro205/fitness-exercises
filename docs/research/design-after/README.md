...
## 웜업·스트레칭 동작 설명 전수 보강 (v55 · PR #65)

- `11-warmup-guide.png` — 라이브 v55, 가로 393px
- `11b-warmup-guide-320.png` — 같은 화면 가로 **320px**(가장 좁은 폰 기준 읽힘 확인)

설명이 한 줄(`cue`)뿐이라 **그 동작을 이미 아는 사람만** 따라할 수 있었습니다. 36개 동작을
**준비 → 동작 → 기준** 세 줄로 다시 썼고, 이름도 목적이 먼저 읽히게 바꿨습니다.

| | 전 (v54) | 후 (v55) |
|---|---|---|
| 이름 | `벽 무릎 터치 (발목)` | `발목 풀기 — 무릎으로 벽 밀기` |
| 준비 | (없음) | 벽을 보고 서서 두 손으로 벽을 짚어요. 앞발 발끝을 벽에서 5cm 떨어뜨리고 다른 발은 뒤에 둬요. |
| 동작 | 발뒤꿈치를 떼지 말고 무릎으로 벽을 터치한다. | 뒤꿈치를 바닥에 붙인 채 무릎을 앞으로 밀어 벽에 대요. 뒤꿈치가 뜨면 거기까지예요. |
| 기준 | (없음) | 한쪽 10회씩이에요. 쉬우면 앞발을 조금 더 뒤로 옮겨요. |

표준 수행법은 동작마다 웹 조사로 확인했습니다(Physiopedia · ACE · AAOS · HSS · FMS · McGill · NASM).
조사 → 초안 → 독립 검증(다른 에이전트가 출처 재확인) 순서로 돌렸고 **36개 전부** 검증 단계에서
수정이 붙었습니다. 그중 2건은 그대로 두면 동작이 안 되거나 반대로 되는 오류였습니다 —
사이드 브리지의 손 위치(자기 어깨에 자기 손), 밴드 외회전의 잡는 팔(반대로 잡으면 내회전 운동).

화면 쪽 최소 조정 두 가지:
- 설명 카드가 `준비 / 동작 / 기준` 세 줄(왼쪽 라벨 + 본문)로 나뉩니다.
- 카드가 길어져 320×568에선 `완료`가 접힘선 아래로 내려가므로 **진행 버튼 줄을 sticky**로 바꿨습니다.
  횟수 구간은 눌러야 넘어가기 때문입니다. 화면에 다 들어오면 아무 일도 일어나지 않습니다.

### 라이브 확인 결과 (v55)

- `APP_VERSION` **v55** 서빙 확인
- 웜업 가이드 393px · 320px 렌더 — 콘솔 에러 0건, 설명 잘림 없음, `완료` 버튼 화면 안
