...
// ═══ 7. Sonnet 5 생각 블록 대응 (핫픽스 잠금) ═══
test('응답 파싱: thinking 블록이 앞에 와도 텍스트를 찾는다 (v38 "응답 비어있음" 오판 방지)', () => {
  const withThinking = { content: [
    { type: 'thinking', thinking: '', signature: 'abc' },
    { type: 'text', text: '오늘은 PULL 추천' }
  ] };
  assert.equal(app.getResponseText(withThinking), '오늘은 PULL 추천');
  // 텍스트 블록이 여러 개면 이어붙임
  assert.equal(app.getResponseText({ content: [{ type: 'text', text: 'A' }, { type: 'text', text: 'B' }] }), 'AB');
  // 진짜 빈 응답은 빈 문자열
  assert.equal(app.getResponseText({ content: [{ type: 'thinking', thinking: '' }] }), '');
  assert.equal(app.getResponseText(null), '');
});
