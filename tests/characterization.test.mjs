...
// ── 묶음6-A: 디자인 토큰화 + 한글폰트(Pretendard) + Inter 제거 ──
...
test('묶음6-A 디자인 토큰 — :root 색 토큰 정의 + var() 적용, 직접 hex 누수 없음', () => {
  ...
  assert.match(css, /--accent:\s*#00d4ff/i, '--accent 토큰이 정의돼야 함');
  assert.match(css, /--accent-rgb:\s*0,\s*212,\s*255/i, '--accent-rgb 토큰(투명도용)이 정의돼야 함');
  ...
  const bareAccent = (css.match(/#00d4ff(?![0-9a-f])/gi) || []).length;
  assert.ok(bareAccent <= 1, `#00d4ff 직접 사용은 토큰 정의 1곳만 허용 (현재 ${bareAccent})`);
});

test('묶음6-A 한글폰트 — index.html에 Pretendard 로드, 본문 폰트 토큰에 Pretendard, Inter 제거', () => {
  const html = fs.readFileSync(path.join(DIR, '..', 'index.html'), 'utf8');
  assert.match(html, /pretendard/i, 'index.html 이 Pretendard 웹폰트를 로드해야 함');
  const css = fs.readFileSync(path.join(DIR, '..', 'css', 'styles.css'), 'utf8');
  assert.match(css, /--font-body:[^;]*Pretendard/i, '본문 폰트 토큰에 Pretendard 가 포함돼야 함');
