...
// ═══ 9. 세트 구성 = 추천 카드 일치 (폰 QA 반영) ═══
test('세트 구성: 기록 있으면 추천 무게 그대로 (5kg 유지인데 세트가 6kg 되는 불일치 방지)', () => {
  seedLog([workout(daysAgo(2), '덤벨 사이드 레터럴 레이즈', [set(5, 25), set(5, 25), set(5, 25)])]);
  const plan = app.getSessionSetPlan('덤벨 사이드 레터럴 레이즈', 5, '15-20');
  assert.equal(plan.weight, 5, '지난 실제 무게 5kg 보존 (2kg 격자로 6kg 반올림 금지)');
  assert.equal(plan.reps, 20, '유지 모드 기본 횟수 = 지난 최고 반복(25)을 범위 상단(20)으로 클램프');
});

test('세트 구성: 기록 없으면 AI 무게를 장비 단위로 스냅', () => {
  seedLog([]);
  app.storage.set(app.KEYS.ONE_RM_DATA, {});
  const plan = app.getSessionSetPlan('덤벨 사이드 레터럴 레이즈', 5, '15-25');
  assert.equal(plan.weight, 6, '기록 없을 땐 스냅된 AI 무게 (덤벨 2kg 격자)');
  assert.equal(plan.reps, 15, '기록 없으면 범위 하단부터');
});

test('세트 구성: 증량 모드면 새 무게 + 범위 하단부터 (더블 프로그레션 리셋)', () => {
  seedLog([
    workout(daysAgo(2), '레그 프레스', [set(100, 12), set(100, 12)]),
  ]);
  const plan = app.getSessionSetPlan('레그 프레스', 100, '8-12');
