...
## 마지막 한 일 (2026-09-03 — 세션 29: 세트 구성 규칙 v69 — 워밍업 내림·경량 백오프·하한 미달 감량, PR #86)

- **사용자 스펙("세트 구성 규칙": 종목 메타 → 탑세트 무게 → 워밍업 → 백오프 → 출력 검증)을 기존 `getSessionSetPlan` 엔진 위에 얹었다.** 새 엔진 없이 빠진 규칙만 추가. 계획 파일 `~/.claude/plans/idempotent-strolling-feather.md`.
  - **사용자 결정 4건(질의로 확정)**: 바벨도 **5kg 격자 유지**(스펙의 2.5 거절) · 탑세트+백오프는 **고중량 복합만**(Angleri 2017: 세트법 간 근비대 차이 없음) · **세트별 RPE 입력 안 넣음**(증량 판정은 반복수만) · 반복 범위는 **클래스값 유지**(한쪽씩 복합만 8-12). 메모리 `user-training-profile.md`에 기록 — 다시 묻지 말 것.
  - **구현**: `unilateral` 메타 7종(런지·불가리안·싱글 레그 데드·원 암 ×3) + `UNILATERAL_REP_RANGE` · 워밍업 램프 **50%×8 → 70%×4 → 85%×2, 격자 내림, 마지막 단 < 탑 88%**(`WARMUP_RAMP`·`WARMUP_MAX_PCT`·`floorWeightToEquipment`; 덤벨·한쪽씩은 2단 50/75) · 백오프 비율 **종목별 90% / 경량(덤벨·한쪽씩·케이블·고립) 85%**, 자동 디로드 85%/80%(`backoffPctFor`·`backoffDeloadPctFor(name, 스킴 선언값)`) · **같은 무게로 2세션 연속 하한 미달 → 한 단계 감량**(`source:'regress'`, `REGRESS_SESSIONS`), 1회는 유지+안내 · 유지일 목표 = 지난 최대 +1(`repsFloor`로 지난 달성치를 바닥으로 넘겨 자동 디로드 오발동 방지) · 기록·1RM 없으면 같은 부위 1RM 추정 폴백(맨몸·어시스트·밴드 제외) · `checkSetPlanInvariants` + 종목×세트법×무게 스윕 테스트(격자 밖 탑 22.5 포함). `FEEDER_PCT/REPS` 삭제(램프 표로 흡수). 골든 심볼 503→509, v68→**v69**.
- **★병합 전 적대적 리뷰(opus 워커)가 테스트 통과 코드에서 9건 지적, 6건 수정** — 차단급 2건: ①같은 부위 1RM 폴백이 **맨몸·어시스트 종목에 kg을 처방**(플랭크 30kg·풀업 45kg 탑세트 — 신규 사용자 기본 상태에서 발생; builder가 characterization 테스트의 1RM을 비워 우회한 흔적이 단서) ②유지일 "+1 목표" 때문에 **지난 세션과 똑같이 해도 자동 디로드가 매번 발동**. 그 외: 격자 밖 탑에서 검사기 오탐, 프롬프트에 감량 규칙 누락, `SET_SCHEMES.autoDeload.pct`가 죽은 선언, 한쪽씩 교집합이 8-8로 퇴화, 묵은 주석. **보류 2건(다음 세션 판단)**: ⑦배포 직후 진행 중이던 세션(v68 90% 백오프 저장분)은 경량 종목 자동 디로드가 1회 건너뜀 ⑧통증 게이트가 감량(regress)보다 먼저 반환해 "아프면서 2세션 미달"은 감량 안 됨 — 의도 확인 필요.
- **검증**: `node --test` 11파일 **512/512** · verifier 하네스 스팟체크(60→30/40/50·55, 45→20/30/35·40, 덤벨 불가리안 20→10/14·18, 2세션 미달→55, 유지일 9회) 전부 일치, 스윕 1,800조합 위반 0 · 헤드리스 브라우저 실제 클릭: 하체 세션 탑 60/백오프 55·55, 탑 3회 완료 → 백오프 50 자동 디로드, 콘솔 에러 0.
- **★범위 밖 관찰(기존 코드, 미수정)**: API 키 없는 폴백 루틴은 모든 종목 `isMain:false`라 고중량 복합에도 **워밍업 램프가 안 붙음**(`js/screens.js:341`). 또 `prog.note`(감량 안내 등)는 화면에 렌더되지 않고 코치 컨텍스트로만 감 — 무게가 말없이 60→55로 내려간다. UI 표시 여부는 사용자 결정 사항.
- **★교훈**: ①builder가 "기존 테스트를 고쳐 통과시켰다"고 하면 그 테스트가 **왜** 깨졌는지 본다 — 이번 차단급 버그 ①의 단서가 정확히 거기 있었다. ②스펙 예시값을 그대로 기대값으로 적지 말고 손으로 계산한다(20×0.85=17→2kg 반올림 18, 계획서엔 16으로 적었다가 지시서에서 정정).

### 이전 (2026-09-02 — 세션 28: AI 코치 지식·규칙 전면 재검증, v67→v68 배포)

- **grill-me 9문답으로 범위 확정** → 조사 6갈래(Opus 병렬) → 검산(코드 주장 18건 verifier, 논문 32건 collector·직접 확인) → 선별안 승인 → PR 3개 순차 병합·배포.
  - **PR #82 문서**(`bad5667`): `docs/research/v2-*.md` 6건(볼륨·빈도 / 강도·진행 / 종목·자극 / 디로드·정체·회복 / 유산소 / 세트법·휴식·웜업) + `v2-selection-plan.md`(A 정합성 10 · B 코드값 6 · C 문구 7 · D 기능 결정). 낡은 절 정정: 허리 금기 9→코드 실측 4, `axialLoad` 미구현 명시, 경사 걷기 §6 배치 서열 폐기(Schumann 2022·Held 2026).
  - **PR #83 지식·프롬프트**(`0648697`, v67): `COACH_KNOWLEDGE` 9절→7절 재작성 — **영양 §4·5·6 삭제(사용자 결정)**, §7 유산소 병행 신설, Pelland 2026·Robinson 2024·ACSM 2026·Havers 2026·Singer 2024·Warneke 2024·Coleman 2024/Pancar 2026 근거로 갱신. 정합성: 디로드 트리거 "3세션" 제거, 디로드 RIR 3~5/3~4→3~4 통일, "작은 근육은 간접자극으로 목표 낮다" 4곳 삭제(이중 차감), 렉스트포즈 오타, 증량 트리거 문구 1~2세션. 코치 원칙 5 = 영양 질문은 "훈련 외 주제"로 넘김. 실제 API 7항목 합격.
  - **PR #84 코드 규칙·기능**(`9e6c104`, v68): 반복 범위 통일(고중량 복합 5-8→**6-10**, 고립 12-15→**10-15**, 경량 고립 15-25→**12-25** — 클램프가 프롬프트 지시를 자르던 문제), 첫 무게 0.7 일괄→클래스별 **0.78/0.72/0.66/0.60**(Nuzzo 2024), 증량 폭 10% 초과 **경고**(차단 아님 — 5kg 덤벨은 막으면 영영 증량 없음), 작은 근육 볼륨 목표 8→**10**·수확 체감선 16→**20**(Pelland 2026), **직접 세트 하한 4**(팔·측면/후면 어깨·종아리 → `directShort`), 손잡이 `hold`→`hold_upright`(유지)/`hold_lean`(−2%, Hofmann 2014), 신장 태그 3건 제거, 코치 컨텍스트 정체 마커 = 정체 엔진(4세션·28일) 재사용, 볼륨 폐루프에 **이번 주 누적** 병기, RPE 회복 신호 = 개인 기준선 +1.5("오버트레이닝" 라벨 삭제), 홈 카드 **디로드 앞당기기 제안**(정체 2종목↑, 최근 디로드 21일 내 억제), 걷기 엔진이 어제·오늘 하체 세션 인식, 죽은 "오늘의 추천" 엔진·키 삭제, main부터 실패하던 복원 테스트 수리.
- **★독립 리뷰(Orca 워커 claude-opus-5·max)가 병합 전 15건 지적** — 크래시 0·삭제 잔재 0이었지만 **볼륨 임계 변경이 ai.js 5곳에 안 퍼져 한 요청 안에서 자기모순**(F1·F2), 프롬프트 규칙 7이 B2 완화 전과 어긋남(F3), 디로드 직후 재제안(F4), 기록 화면이 `split.direct`를 안 넘겨 직접 세트 부족이 화면엔 안 뜸(M1), 프롬프트 "소근육 12-25" 목록에 팔이 들어 코드(10-15)와 충돌(M3) 등. 전부 후속 커밋으로 반영 후 병합.
- **★교훈**: ①**위임이 오류로 실패해도 직접 하지 않는다** — 에이전트 팀 tmux가 죽으면 `name` 없이 재호출(정답), 그래도 안 되면 Orca 워커(worker-launch). 이번엔 3번 실패 뒤 직접 구현했다가 사용자 지적을 받았다. ②**테스트 전부 통과 + 브라우저 QA + API 1회로도 리뷰어는 15건을 찾았다** — "값 하나를 바꾸면 그 값이 적힌 모든 곳"을 grep으로 전수 확인하기 전엔 끝난 게 아니다. ③**선별안에서 정한 규칙(B2 차단)도 구현 중 근거가 나오면 완화하고 문서·프롬프트·코드 셋을 같이 고친다** — 코드만 바꾸면 프롬프트가 옛 규칙을 말한다.
- **남은 백로그(선별안 D 보류 + 리뷰 사소)**: D1 사이클 5→6주(잠정), D3 관절통 기록, D6 볼륨 점증 제안, D7 레스트포즈·클러스터 스킴, 리뷰 S7(`jumpWarned` 미사용)·S8(디로드 전환 토스트에 진행도 리셋 안내·"건너뛰기 가능" 문구 불일치). 코치 답변 말투가 문항마다 해요체·평서체·반말로 흔들리는 기존 현상(범위 밖).
- QA 키 `~/.healthapp_qa_key`는 세션 끝에 삭제 — **사용자는 Anthropic 콘솔에서 그 키를 폐기·재발급할 것**(대화 기록에 남았음).

### 이전 (2026-08-25 — 세션 27: 여백·정렬 정돈 + 웜업 재설계, v66 배포)

- **지시 4건을 PR #80 으로 처리·병합·배포 완료** (`1f585c1`, 운영 v66, 라이브 QA 20/20).
  ①운동 탭 부위 5장을 탭바 위 남은 영역의 **세로 가운데**로(글자 정렬은 왼쪽 유지 — 사용자 선택) ②상단 '뒤로'에 박스가 붙던 것 정돈 ③루틴 생성 로딩 박스도 같은 기준으로 ④웜업 화면 재설계.
- **★②의 원인은 눈대중이 아니라 "CSS 에 없는 클래스"였다.** 이 저장소는 Tailwind 를 안 쓰고 **같은 이름의 유틸을 손으로 적어 둔다** → 이름이 없으면 오류도 경고도 없이 **아무 일도 안 일어난다**. 두 갈래로 나 있었다:
  - **정의를 통째로 빼먹음** — `pb-2·pb-3·pb-4·pb-20·pt-5·mt-5·mt-6·mb-6·py-4` 아홉 개가 **정의 없이 18곳**에서 쓰였다. 2단계 헤더 `pb-2` 가 0px 이라 '뒤로'와 아래 박스 간격이 **실측 1px**.
  - **HTML 에 CSS 이스케이프를 그대로 씀** — `class="mt-0\.5"` 의 클래스 이름은 백슬래시까지 포함한 `mt-0\.5` 라 `.mt-0\.5`(= 이름 `mt-0.5`)와 **영원히** 안 맞는다. 실측 `margin-top: 0px`. **51곳.**
  - 여기에 `viewport-fit=cover` 를 켜 놓고 `env(safe-area-inset-top)` 을 **어디서도 안 써서**(코드 전체에 safe-area 사용처가 bottom 1곳뿐) 폰에서 상태바가 헤더 위에 겹쳤다. 2단계 `← 뒤로` 는 13px 글자 표적 → 44px 로.
  - 실측: 2단계 뒤로↔첫 박스 **1px → 12px**, 로딩 박스 **0px → 209px**, 3단계 21 → 33px, 1RM 목록 114 → 134px.
- **④ 웜업·스트레칭**: 동작 이름 **36개**를 `부위 — 동작` 에서 **동작 하나**로(`어깨뼈 풀기 — 바닥 밀어내기` → `바닥 밀어내기`, 두 줄로 접히던 게 한 줄). **이름을 주인공으로 재배치** — 이름 32px → 분량 24px 한 줄 → 진행 바(옛 배치는 `남은 시간` 라벨 + 72px 숫자가 화면을 먹고 이름이 뒤로 밀렸다). `천천히 · 반동 없이 …` 줄 삭제, `표준 6분 / 짧게 3분` 토글 + 죽은 `setWarmupMode` 삭제. **짧은 계획을 만드는 도메인 능력(`buildWarmupPlan({short:true})`)과 그 테스트는 그대로 남긴다.**
- **★병합 전 적대적 리뷰(5렌즈 × 반박검증 2인, 45에이전트)가 20건 중 17건 생존 → 브라우저로 전부 재현 후 5건 수정**:
  - **safe-area 를 절반만 적용했었다**(4렌즈가 각자 수렴) — `.pt-12` 만 고치고 `.coach-header`(1RM 목록·기억 노트·코치 채팅·주간 리뷰·정체기 **8화면**)는 48px 그대로 → 폰에서 탭↔오버레이를 오갈 때 상단이 튄다.
  - **로딩 화면 42px 헛스크롤** — 남은 높이를 재는데 바깥 래퍼의 `pb-32`(128px)가 또 더해져 화면 높이를 **두 번** 셌다(실측 `문서 886 / 화면 844`).
  - **여백 말고도 정의가 없던 유틸** — `.text-3xl` 이 없어 로딩 카드 `PUSH` 가 28px 이 아니라 **14px**(6곳), `.grid-cols-2` 가 없어 운동 중 `이전/다음 종목` 이 2열이 아니라 **위아래로 쌓임**. `.text-lg·.text-left·.leading-tight·.tracking-wider·.card-warning` 도 추가. 죽은 `.food-input-header`(삭제된 '연료' 잔재) 정리.
- **★처음 만든 가드 테스트가 허수였다** — 백슬래시 51곳을 **못 잡는데도 통과**했다(HTML·CSS 양쪽에서 백슬래시를 지우고 비교해서). 비교를 **HTML 에 적힌 그대로**로 바꾸고, 검사 범위를 여백에서 크기·정렬·격자 유틸까지 넓히고, 문자열 연결로 만든 `class` 속성도 앞부분까지 보게 했다. safe-area 검사도 `.pt-12` 하나에서 **최상단 여백 전부**로. **네 경우를 일부러 되돌려 실제로 잡히는 것을 확인**했다.
- **QA 하네스 결함** — 헤드리스에서 `localStorage.clear()` + `location.reload()` 만으로는 **서비스워커가 옛 `js/data.js` 를 계속 먹인다**(SW 는 HTTP 캐시와 별개로 fetch 를 가로챈다). 고친 이름이 화면에 안 떠서 코드를 의심할 뻔했다 → CDP `Network.setBypassServiceWorker` 를 켜야 실제 코드를 본다.
- **★교훈**: ①**"고쳤는데 화면은 그대로"의 원인 후보에 '클래스가 CSS 에 없다'를 넣는다** — 이 저장소는 유틸이 수제라 오타·누락이 조용히 무효가 된다. ②**HTML class 속성은 CSS 이스케이프를 풀지 않는다** — `class="mt-0.5"` 로 쓰고 CSS 에서만 `.mt-0\.5` 로 이스케이프한다. ③**가드 테스트는 결함을 되돌려 실제로 잡히는지 확인하기 전까지 허수일 수 있다.** ④**한 곳만 고치고 "고쳤다"고 하지 않는다** — safe-area 를 `.pt-12` 에만 넣고 같은 역할의 `.coach-header` 8화면을 빠뜨렸다.

### 이전 (2026-08-25 — 세션 26: 화면 간소화 8건, v65 배포)

- **화면별 지시 8건을 PR #78 로 처리·병합·배포 완료** (`525b5e1`, 운영 v65, 라이브 QA 35/35).
  - **2단계**: AI 분석 완료 배지·다시 분석(↻)·단계 표시/진행 점·큰 부위명+헤드라인·주의사항 박스·종목명 아래 `복합 · {AI 설명}` 줄 삭제. 한 종목 = **이름줄 + 처방줄**, 세트법은 줄 끝에 짧게. 요약은 `← 뒤로` 오른쪽 한 줄. **줄 전체가 종목 편집 시트를 연다.**
  - **3단계**: 퀵칩 5개 삭제. **운동 중**: 상단 버튼 3개 → **톱니바퀴 하나**(코치·세트법은 그 메뉴로), 종목명 위 `현재 종목`·아래 `타입 · 세트법 · 휴식 N초` 삭제, **무게 카드 통째 삭제**, `+ 종목 추가` 삭제.
  - **홈**: 오늘의 추천 삭제 + 배경 API 호출 중단. **운동 탭**: 이번 주 기록 삭제, 부위 5개를 **2열 그리드 + FREE 가로** → 스크롤 없이 정확히 한 화면(844/844px). `이번 주 N`·`추천` 배지 삭제.
- **★사용자 지적으로 드러난 앞 세션의 실패**: 세션25에서 "2단계에서 종목을 눌러 수정"을 만들었다고 보고했는데, 실제로는 **3단계 대화 화면 안쪽 접힌 목록**에 달아 놓아 사용자가 도달할 수 없었다. 원인 둘 — ①탐색 때 **"지금 탭이 엉뚱하게 동작하는 곳"(3단계 인체도)** 을 기준으로 삼고 "사용자가 AI 추천을 실제로 보는 곳"(2단계)을 기준으로 삼지 않음 ②QA 가 `state.workoutWizardStep=3; routinePreviewExpanded=true` 로 **기능을 가리던 두 단계를 정확히 건너뛰고** 45/45 통과를 봄.
- **★같은 실수를 이번에도 반복했다가 잡음**: 종목 줄에 `openExerciseEdit` 를 달아 놓고 **시트를 2단계 반환값에 안 붙여** state 만 바뀌고 화면엔 아무것도 안 떴다. **상태를 세팅하지 않고 PUSH 를 실제로 클릭하는 QA** 로 바꾼 뒤에야 드러났다.
- **작업 중 자체 사고 3건**: `renderAIRecommendationCard` 삭제 시 블록 경계를 잘못 잡아 **`renderWorkout`·`gradeColor`·`GRADE_COLORS` 까지 함께 삭제**(복구) · 편집 시트로 종목을 다 빼면 `[시작]` 이 살아 있고 요약이 `0종목 · 18세트` 로 거짓말 · `.ex-name-flag` CSS 누락.
- **★병합 전 적대적 리뷰(5렌즈 × 반박검증 2인, 16에이전트)가 확정 8건**을 잡아 전부 수정:
  - **안전(3렌즈 수렴, 반박 5인 전원 실패)**: `routine.caution` 은 AI 주의 1문장만 담는 필드가 아니라 부상 가드레일 `🛡️ 안전 교체: 바벨 스쿼트 → 핵 스쿼트 (허리 보호)` 와 장비 가드레일을 **같은 문자열에 이어 붙이고** 있었고, 2단계 주의사항 박스가 **앱 전체의 유일한 표시 자리**였다. 지우자 앱이 부상 때문에 종목을 바꿔 놓고 그 사실을 말하지 않게 됐다. → `js/ai.js` 에서 `caution`(AI 산문) / `swaps`(앱이 한 일)를 분리해 후자만 되살림.
  - 헤더 요약이 같은 화면의 편집을 안 따라옴(출처가 둘) · 미리보기 교체·빼기 `saveWizard()` 누락 · `describeSetStructure` 가 앱에서 죽었는데 테스트 4개가 통과(세트법 시트로 이동) · `오늘의 추천` 테스트가 배경 API 호출 부활을 못 잡음 · 통증 프리필 테스트의 `progOf()` 가 행위 전후 구분 못 함(순수 함수) · `refreshAIRecommendation` 도달 불가 · 이번 PR 이 고아로 만든 CSS 21블록.
- **사용자 판단**: `맨몸 졸업`·`통증 — 증량 보류`·`재활 — 무게 유지` 라벨은 **되살리지 않는다**(동작은 그대로, 설명만 없음). `안전 교체` 알림과 `기본 루틴 — API 키` 안내는 **되살린 게 맞다**.
- **★교훈**: ①**기능을 어느 화면에 다는지는 "코드가 어디 있나"가 아니라 "사용자가 어디를 보나"로 정한다.** ②**QA 는 state 를 세팅하지 말고 실제로 클릭해서 한다** — 상태 강제는 자기가 만든 구멍을 정확히 건너뛴다. ③**새 시트를 만들면 반환값에 붙였는지 반드시 확인**(state 플래그만으로는 안 뜬다). ④**한 필드에 성격이 다른 정보가 얹혀 있는지 지우기 전에 본다**(caution = AI 산문 + 앱의 안전 조치).

### 이전 (2026-08-25 — 세션 25: 사용자 제보 9건 처리, v59→v64 배포)

- **사용자 제보 9건을 PR 4개로 처리·병합·배포 완료** (`d2861a3` #73 · `cd0e7d2` #74 · `a05a6d4` #75 · `7cdcedd` #76). 운영 v64, 라이브 QA 45/45 통과.
- **★가장 큰 발견 — 제보 #5(이두 추천이 리버스컬로 쏠림)와 #6(볼륨의 전완·요추)은 원인이 하나였다.** `이지 바 리버스 컬`이 데이터상 이두가 아니라 **전완 종목**(`primary:'forearms'`)인데, 전완은 직접 종목이 앱 전체에 그것 하나뿐이라 주간 볼륨이 **구조적으로 영구 '부족'**이었다. `buildUserContext`가 부족 부위마다 권장 종목을 붙이므로 **모든 AI 호출**의 프롬프트에 `전완 부족 → 권장 종목: 이지 바 리버스 컬`이 매번 실렸다. 게다가 코치 채팅 프롬프트엔 종목 목록이 하나도 없어 모델이 프롬프트에서 본 유일한 팔 종목을 읊었다. `BODY_PART_GROUPS`·`WEAK_PART_EXERCISE_MAP` 4줄 삭제 + `buildExerciseCatalogBlock` 주입으로 둘 다 해결.
- **요추는 직접 종목이 0개** — 막대가 항상 0이라 영구 빨간 '부족'. 척추기립근은 스쿼트·데드에서 사실상 등척성이고 RP 등 표준 볼륨 랜드마크에도 없다. 근거·출처는 `docs/research/volume-parts-and-exercise-catalog.md`.
- **빠져 있던 유명 종목 6개 추가**: 라잉 트라이셉스 익스텐션(별칭 스컬크러셔)·덤벨 오버헤드 트라이셉스 익스텐션·덤벨 컬·바벨 벤치 프레스·바벨 오버헤드 프레스·바벨 슈러그. **프리웨이트 삼두 고립이 하나도 없었다**(고립 3종이 전부 케이블).
- **#4 지난 기록 부정확** — 대표 무게 하나에 전 세트 횟수를 붙여 `40kg × 4·10회`가 됐다. 같은 표기가 **코치가 받는 세션 컨텍스트에도** 있어 AI까지 오해하고 있었다. `formatSetsSummary` 단일 원천으로 세 곳 통일.
- **#1·#2** — 미리보기·운동 중 공용 **종목 편집 시트**(무게·목표 반복·세트·쉬는시간·종목 바꾸기/빼기) 신설. 코치는 이제 "바꿨어요" 거짓말 대신 **[적용] 승인 카드**로 제안하고, 적용은 편집 시트와 **같은 함수**를 탄다.
- **#8·#9** — 화면이 자기 UI를 설명하던 줄 삭제 + `/humanize`(humanize-korean, light) 윤문(변경률 18.9%·등급 A). **가드 테스트 3종 신설**(해요체·느낌표·40자) — 예외 목록 없이 통과.
- **★병합 전 적대적 리뷰가 결함 8건을 잡았다** (Workflow 6렌즈 × 반박검증 2인, 17에이전트, 1.57M 토큰). **전부 하네스로 실물 재현 확인 후 수정**(`759ae6a`):
  - **크래시**: `exerciseEditValues`/`applyExerciseChange`가 인자 대신 전역 `state.exerciseEdit`으로 갈래를 판단 → 코치 [적용]이 미리보기 종목을 가리키면 `render()`가 통째로 죽음. → `isPreviewExercise(ex)`로 **객체 모양** 판별.
  - [세트] 스테퍼가 완료 세트 이중 계산([+1]이 2개 늘고 [−1]이 오히려 늘어남) · 탑세트 완료 후 무게 스테퍼 얼어붙음 · 반복·세트만 만져도 무게가 한 단 하락 · 미리보기 편집 `saveWizard()` 미호출 · `parseCoachApplyBlock`이 잘린/꼬리 붙은 블록에서 원문 JSON 노출 · 스와이프 가드 누락 · 처방 줄 휴식 미반영.
- **드러난 사각지대**: `EXERCISE_SAFETY[].mod`는 `data.js`에 있지만 **부상 토스트로 화면에 뜬다**. 문구 가드가 `data.js`를 제외해 **111문장 위반을 못 잡고** 있었다. 부상 안내라 일괄 재작성은 뜻이 바뀔 위험이 커 보류하고 **개수가 늘지 않게** 못박는 테스트를 넣었다.
- **★교훈**: ①**stacked PR + squash merge는 매 병합마다 리베이스가 필요하다** — main 해시가 바뀌어 위 PR이 CONFLICTING이 된다. `git rebase --onto origin/main <옛 부모> <가지>` 로 해결. ②병합 전 적대적 리뷰가 값을 했다 — 테스트 476개를 통과하고 브라우저 QA까지 끝낸 코드에서 **크래시 포함 8건**이 나왔다. 여러 렌즈가 **독립적으로 같은 결함**에 수렴한 것이 강한 신호였다. ③새 시트를 만들면 등록할 곳이 **6군데**다(state·render 꼬리·getTopLayer·navBack·세션 종료 정리·휴식 틱 가드·**스와이프 가드**).

### 이전 (2026-08-08~09 — 세션 24: 오케스트레이션 대규모 업그레이드, v41→v57 배포 19건)
- **운영 체계**: Orca 오케스트레이션 Run 1개로 작업 23건 등록·전원 완료. 연구(Opus·max) → 코디네이터(Fable) 선별 → 승인 → 구현(Opus·ultracode) 파이프라인. 워커 ask/reply 실작동(반올림 질문→2분 회신). 완료 즉시 터미널·워크트리 정리(메모리 렉 예방).
- **연구 보고서 7건** → `docs/research/`: set-schemes(v1·v2)·incline-walking·ai-coaching-audit·training-splits·warmup-stretching·design-audit(+스크린샷).
- **배포 19건 (전부 PR·리뷰·테스트·라이브 QA 통과)**:
  - 허리 금기 완화(#42 v41, 세션23 기록 참조) → 러시안 트위스트 contra 복원(#45에 포함)
  - 장비 태그+종목 93→117(#45 v42) · AI 코칭 6항목/별칭버그(#49 v45) · 데이터 백업(#44 v43) · 인클라인 걷기 모드(#46 v46) · 근육 인체도 117종목(#50 v44) · 세트법+휴식+분할통합+슈퍼세트(#52 v47) · 운동 중 종목 추가/건너뛰기(#53 v48) · XSS 이스케이프 21곳(#57·58 v51) · AI응답 숫자 타입 강제(#61 v53) · 미리보기 처방표 복원+탑세트 표기(#62·63 v54) · 디자인 정돈 44건+결함6(#59·60 v52) · 웜업/스트레칭 가이드(#54 v49) · 어시스트 역방향 과부하(#55 v50) · 드릴 문구 36개 재작성(#65·66 v55) · AI루틴 메인복합 우선(#68 v56) · 세트법 v2 7종+무게 반올림(#67·69 v57)
- **세트법 최종 상태(v57)**: 시트 7종(스트레이트/탑+백오프/탑+백다운/피라미드/역피라미드/드롭/마이오렙). 탑+백오프 = 피더단(90%×2회)+마지막 백오프 RIR0~1+탑 미달 시 0.85 감량. 탑+백다운 = 탑 100%×8~12 → 82.5%×12~15×RIR0~1×2세트(사용자 원안 70%는 RIR5~6 자극미달이라 교정). 반올림 = 5kg 배수(덤벨 2kg), 감량세트는 가까운 값+기준보다 최소 한 단위 아래 보장. 15,610조합 전수 테스트.
- **프로세스 자산(저장소 밖)**: `~/.claude/skills/worker-launch/`(착수 자동화 스크립트+스킬, 5회 실전 검증) · `~/.claude/CLAUDE.md` 워커 운영 규칙 5종(모델정책·설정변경 절차·정리·질문 규칙) · Codex 0.131→0.147 업데이트(구버전 디코드 에러가 리뷰 hang 원인).
- **QA 체크리스트 강화(#51)**: "배포 후 라이브 브라우저 QA" 필수 섹션 신설 — 이후 전 워커 준수.
- **★교훈**: ①워커 보고서는 재료 — 코디네이터가 선별(클러스터 보류·레스트포즈 제외 사례) ②/model은 사용자 기본값을 덮어씀 ③Codex hang이면 버전부터 ④terminal close로는 codex broker가 안 죽음(별도 kill).

## 다음 할 일
- [ ] **폰 실기기 QA (v69 강력 새로고침)** — 하체 세션에서 워밍업 3단(50/70/85 내림)·백오프, 덤벨 종목을 탑세트로 바꿔 85% 백오프, 탑세트 미달 후 자동 디로드
- [ ] 세션 29 보류 2건 판단: 통증 게이트 vs 감량 우선순위 · 감량 안내(`prog.note`)를 화면에 보일지
- [ ] 폴백 루틴(API 키 없음) `isMain:false` → 고중량 복합 워밍업 램프 누락(기존 코드) 고칠지
- [ ] **폰 실기기 QA (v65 강력 새로고침)** — ①운동 탭 부위 5개가 한 화면에 ②2단계에서 종목 줄을 눌러 편집 ③운동 중 톱니바퀴에 코치·세트법
- [ ] ~~폰 실기기 QA (v64 강력 새로고침)~~ — 특히 ①종목 카드를 눌러 편집 시트 ②코치에게 "쉬는시간 줄여줘" → [적용] ③운동 중 [지난 기록] 팝업. 서비스워커 교체로 새로고침이 두 번 필요할 수 있음.
- [ ] 코치에게 실제 질문 1회로 API 호출 확인 (키가 폰에만 있어 4세션 누적 미실시)
- [ ] 백로그: `EXERCISE_SAFETY[].mod` 문구 111건 해요체·40자 재작성(부상 안내라 뜻 보존 주의) · 클러스터 세트(보류) · UPPER 후면삼각 고립 없음 · 월 사용비용 표시
- [ ] ~~폰 실기기 QA(v57 강력 새로고침)~~ + 코치에게 질문 1회로 실제 API 호출 확인(3세션 누적 미실시 — 키가 폰에만 있음)
- [ ] 백로그: 클러스터 세트(보류 결정) · UPPER 후면삼각 고립 없음(#52 인계) · 슈퍼세트 자동제안 프리웨이트 확장 · 월 사용비용 표시 기능 · 사용자 PC WSL 메모리 설정(memory=24GB+autoMemoryReclaim, `wsl --shutdown` 필요 — 워커 없는 때)

## 막힌 점 / 주의
- 실제 API 키 호출 검증 미실시(키가 사용자 폰 localStorage에만 존재) — 프롬프트 변경분은 문자열/스텁 검증만.
- 사용자 Claude Code 기본 모델이 워커 작업 부작용으로 Opus 5로 저장돼 있음(세션 시작 시엔 Fable). 원하면 `/model`로 복원.
- sailfish 로컬 브랜치는 main보다 한참 뒤(코디네이터 베이스일 뿐) — 다음 세션 코드 확인은 origin/main 기준으로.

## 마지막 커밋
- PR #86 세트 구성 규칙 v69 (feat/set-rules-v69 → main squash) — 2026-09-03 · 운영 v69
- 525b5e1 화면 간소화 8건 + 리뷰 확정 8건 수정 (#78) — 2026-08-25 · 운영 v65

_다음 세션 재개: "헬스앱 작업 이어서 해줘" — 첫 할 일: **폰에서 v69 강력 새로고침 후 세트 구성 체감 확인**(워밍업 3단 내림 · 백오프 · 탑세트 미달 자동 디로드). 그다음 세션 29 보류 2건(통증 게이트 vs 감량, 감량 안내 표시) 결정._


### 이전 (2026-08-08 — 세션 23: 허리 부상 금지운동 과잉 제한 완화 [브랜치 `ironbro205/injury-logic-fix`])
