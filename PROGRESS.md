...
## 마지막 한 일 (2026-08-08 — 세션 23: 허리 부상 금지운동 과잉 제한 완화 [브랜치 `ironbro205/injury-logic-fix`])
- **배경**: 허리(`lower_back`) 부상 등록 시 `EXERCISE_SAFETY`의 `contra` 태그로 **9종목이 전부 추천 금지** → 사용자(허리디스크 보유, 초중급)가 과잉 제한이라 판단하고 사양 확정.
- **근거 조사 = 워크플로우** (5주제 병렬 + 종합, 6에이전트/583k토큰): 스쿼트·데드리프트 금기 방어 가능성 / RDL·로우 하향 타당성 / 코어 굴곡+회전 / 데드리프트 대체 / 스쿼트 대체.
- **확정 등급**: 금기 **3종**(바벨 스쿼트·프론트 스쿼트·데드리프트), 주의 **6종 하향**(루마니안 데드리프트·바벨 루마니안 데드리프트·바벨 로우·T 바 로우·러시안 트위스트·머신 시티드 크런치). 대체 종목 교체 — 스쿼트류 `레그 익스텐션`(고립·메인불가) → **`핵 스쿼트`**(복합·메인가능), 데드리프트 `라잉 레그 컬` → **`시티드 레그 컬`**(Maeo 2021 신장강조 +14% vs +9%). RDL 3변형 sub도 시티드 레그 컬로 통일.
- **★근거로 통념 4건 폐기**: ①"핵 스쿼트 등판이 축하중 제거" = 근거 없는 벤더 주장(실제로는 몸통 굴곡 모멘트 제거 → 기립근 요구 감소, Clark 2019) ②"RDL이 데드리프트보다 요추 부하 낮다" = 동일조건 직접 비교 연구 없음 ③"랜드마인이라 허리 친화적" = 전통 RDL과 요추 ROM·근활성 차이 없음 ④butt wink → 디스크 손상 직접 근거 없음(Cappozzo 1985 실측치로 교체). 널리 인용되는 "Gullett 2009이 프론트 스쿼트 요추 부하 낮음을 보였다"는 **오인용**(그 논문은 무릎 관절 측정).
- **구현**: `js/data.js` EXERCISE_SAFETY 데이터 변경만으로 충분 — 등급 판정·프롬프트 주입·VETO 가드레일이 모두 데이터를 읽는 구조. 부수 효과로 `WEAK_PART_EXERCISE_MAP`이 권장하던 종목을 안전 블록이 동시에 금기로 막던 **프롬프트 모순 3건 해소**.
- **Codex 리뷰 2라운드 → 2건 수정**: ①금기를 caution 대체 종목으로 바꿀 때 그 대체의 `mod`가 사용자에게 전달 안 됨(레그 익스텐션 → 핵 스쿼트로 바꾸며 노출된 경로) ②그 수정이 `checkExerciseSafety(sub, areas)`를 통째로 호출해 **다중 부상 시 caution끼리 덮어써** 교체 사유와 다른 부위 문구가 붙음 → 부위별 개별 조회 + 교체 사유 부위 우선 정렬로 해결. 2차 재확인 `resolved`.
- **검증**: 테스트 101 → **104/104**(안전 3건 신설). 5개 탭 헤드리스 렌더 통과. 변경 전후 실측 — 루틴 7종 투입 시 **before 4종으로 붕괴(메인이 레그 익스텐션으로 강등)** → **after 7종 전부 생존, 금기 2종만 교체**. 캐시 v40→**v41**, APP_VERSION 동기화.
- **⚠️ 사용자 판단 대기 2건**(구현은 확정 사양대로 했으나 근거가 어긋남): ①**프론트 스쿼트 금기는 근거상 방어 약함** — 백스쿼트보다 요추 부하가 낮거나 비슷한데, 굴곡 모멘트가 훨씬 큰 로우/RDL을 허용하면서 이걸 막으면 내적 일관성이 깨짐 → caution 재분류 검토. ②**러시안 트위스트 caution 하향은 근거상 정당화 안 됨** — 굴곡+회전은 섬유륜 원주형 균열을 만들어 신전 기반 회복까지 무력화(Marshall & McGill 2010), 무게를 빼도 안 사라지고 회전을 빼야 사라짐. 수정을 다 적용하면 사실상 흉추 회전 드릴이라 이름 유지가 잘못된 안전감을 줄 수 있음 → contra 복귀 또는 팔로프 프레스 계열 반회전 종목 신설 검토.
- 후속 후보(미구현): `WEAK_PART_EXERCISE_MAP`에 안전 필터 없음 — 부족 부위 권장 종목이 caution이어도 그대로 프롬프트에 주입된다(현재는 모순 없음, 등급이 또 바뀌면 재발 가능).

### 이전 (2026-07-14 — 세션 22: 개편 3단계 세트 사이 채팅 [브랜치 `session-chat`])
