...
- **구현은 메인 Claude 직접도, `Workflow` 도구·서브에이전트 병렬 위임도 자유**다(울트라코드 우선 — 전역 규칙). 독립 작업은 적극 병렬화하되, 추적 안 되는 background plain executor 직접 spawn만 피한다. 읽기전용 탐색은 `Explore`, 외부 리뷰는 Codex(또는 폴백 리뷰어)가 담당해 구현자/리뷰자 분리만 유지한다.
