...
## 배포 후 확인 (main 병합 시 필수)

- [ ] **브라우저로 라이브 주소(fitness-exercises-iota.vercel.app)를 직접 열어 확인했다** — Orca 내장 브라우저(`orca tab create`/`snapshot`/`eval`), Playwright, 크로미움 등 환경에서 쓸 수 있는 브라우저 도구 아무거나. 없으면 "라이브 미확인"을 완료 보고에 명시한다.
- [ ] 라이브에서 `APP_VERSION`(새 캐시 버전)이 서빙되고, **이번에 바꾼 기능이 실제 화면에서 동작**하며, 콘솔에 빨간 에러가 없다.
- [ ] 확인 범위는 **변경된 기능 중심 스팟체크**로 한정한다 — 전 화면 전수조사 금지(시간·컨텍스트 비용 통제). 확인 후 열었던 탭은 닫는다.
